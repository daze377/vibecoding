# Headless rule tests (task-23.md §9): damage math, ammo, zone schedule.
...
func _test_zone_schedule() -> void:
	var zone_script := load("res://scripts/zone.gd")
	var radii: Array = zone_script.RADII
	check(radii == ([60.0, 35.0, 18.0, 6.0] as Array[float]), "zone radii match the design doc")
	check(zone_script.DAMAGE_PER_SECOND == 5, "outside damage is 5/s")
